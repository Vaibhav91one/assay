...
//
// WHY THIS IS NOT IN THE SERVER ACTIONS IT SERVES: same reason as
// `web/lib/commands.ts` -- a `'use server'` module reaches `@/lib/auth` and
// `next/cache`, which the root tsconfig does not resolve and a test process
// cannot satisfy. These are the two most dangerous operations in the product, so
// they are the two that most need a test that really calls them. The auth
// wrapper is `web/app/(app)/brake-actions.ts`.
 ...
 */
export async function fieldControlsFor(targetId: string, field: string): Promise<FieldControls> {
 ...
 */
export async function clearBrakeFor(input: {
  targetId: string;
  field: string;
  confirm: string;
  /** Recorded in `brake_reason` as prose. The action supplies the operator. */
  clearedBy: string;
 ...
 */
export async function unhealFieldFor(input: {
  targetId: string;
  field: string;
  /**
