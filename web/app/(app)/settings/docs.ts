 ...
 *
 * Both now have a section of their own. Discord used to point at
 ...
 * `{event, data, sent_at}` envelope that a Discord incoming webhook answers
 * with `invalid_payload`. An operator who followed that button got a working
 * link to a mechanism that cannot do what they came to do, which is worse
 * than a button that scrolls nowhere: it costs them the afternoon before they
 * find out.
 *
 * `#discord` is a real heading now, and it documents the actual credential --
 * an incoming-webhook URL in the connectors file, not a variable in `.env`.
 *
 ...
 *
 * `Kind` is a stored token -- `brightdata`, `discord` -- and the
 * Connections tab was printing it straight into a row label, so the screen said
 * "brightdata · API token" and "discord" beside prose that said "Bright Data" and
 * "Discord" two columns over. The token is not the name.
 *
