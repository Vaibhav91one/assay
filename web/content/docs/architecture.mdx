...
flowchart TB
  subgraph host["one host · docker compose up -d"]
    web["web<br/>Next 16 · App Router<br/>serves screens, writes operator actions"]
  ...
  bd["Bright Data<br/>scrapes on its own schedule"]
  fc["Firecrawl<br/>for pages that refuse a plain request"]

  worker -->|"http"| site
  worker -.->|"http, only after a direct<br/>request was refused"| fc
  bd -.->|"webhook delivery<br/>same ingestPage path<br/>web receives, never fetches"| web
...
|---|---|---|
| `web` | `build: .`, `npm --workspace web run start` | Serves the screens and the REST surface; writes operator actions. Port 3000. |
