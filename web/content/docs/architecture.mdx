...
flowchart TB
  subgraph host["one host · docker compose up -d"]
    ...
    pg[("postgres:17-alpine<br/>twelve tables<br/>volume: pgdata")]
...
|---|---|---|
| `web` | `build: .`, `npm --workspace web run start` | Serves the screens and the read-only REST surface; writes decisions. Port 3000. |
