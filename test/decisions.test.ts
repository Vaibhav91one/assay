...
const RANKED = [
  { selector: 'span.price', score: 0.71, value: '$49.99' },
  { selector: 'span.was', score: 0.62, value: '$69.99' },
];
...
/** Four held cells: three on template A/B, one with no group and no candidates. */
async function seed() {
  const d = getDb();
  ...
  const cells = [
    { field: 'price_1', proofId: P1, groupKey: GROUP_A, ranked: RANKED },
    { field: 'price_2', proofId: P2, groupKey: GROUP_A, ranked: RANKED },
    { field: 'price_3', proofId: P3, groupKey: GROUP_B, ranked: RANKED },
    // No group and only one candidate -- the "second" guard has to see this.
    { field: 'lonely', proofId: SOLO, groupKey: null, ranked: [RANKED[0]] },
  ];
  for (const c of cells) {
    await d.insert(fieldRuns).values({
      runId, field: c.field, value: null, status: 'quarantined',
      reason: 'margin', proofId: c.proofId, ranked: c.ranked,
...
describe('the answer vocabulary', () => {
  it('is a closed set of four', () => {
    expect(Resolution.options).toEqual(['first', 'second', 'empty', 'neither']);
  });
  ...
  it('refuses an unrecognised key rather than ignoring it', () => {
    expect(ResolveInput.safeParse({ proof: 'pr_x', resolution: 'first', force: true }).success)
      .toBe(false);
  });

  it('refuses an empty proof id', () => {
    expect(ResolveInput.safeParse({ proof: '', resolution: 'first' }).success).toBe(false);
  });
});
...
describe('resolving', () => {
  it('refuses a proof that is not in the queue', async () => {
    if (!dbUp) return;
    const r = await resolve({ proof: 'pr_no_such_thing', resolution: 'first' });
    expect(r).toMatchObject({ ok: false, error: 'not_found' });
  });

  it('refuses "second" when the gate ranked only one candidate', async () => {
    if (!dbUp) return;
    // Not a coercion to candidate 1, and not a 200. There is no second.
    const r = await resolve({ proof: SOLO, resolution: 'second' });
    expect(r).toMatchObject({ ok: false, error: 'no_such_candidate' });
    expect(await isOpen(SOLO)).toBe(true);
  });

  it('treats a model nomination as OPEN, not as a settled decision', async () => {
    if (!dbUp) return;
    // Exactly what assay_propose writes: a resolution string, resolved_by null.
    ...
    const r = await resolve({ proof: P3, resolution: 'second' });
    expect(r).toMatchObject({ ok: true, resolution: 'second' });
    ...
    expect(after!.resolution).toBe('second');
  });

  it('applies one answer to every open item on the same template (F8)', async () => {
    if (!dbUp) return;
    const r = await resolve({ proof: P1, resolution: 'first' });
  ...
  it('does not leak the decision to a different template', async () => {
    if (!dbUp) return;
    // P3 was settled by its own decision above, as `second` -- not as `first`.
    expect((await item(P3))!.resolution).toBe('second');
  ...
  it('settles a card once when it is answered twice at once', async () => {
    ...
    const both = await Promise.all([
      resolve({ proof: P1, resolution: 'first' }),
      resolve({ proof: P2, resolution: 'empty' }),
    ]);
  ...
  it('refuses to decide an item that is already settled', async () => {
    ...
    expect((await item(P1))!.resolution).toBe('first');
  });
});
